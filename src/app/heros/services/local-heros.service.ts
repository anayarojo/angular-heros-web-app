...
@Injectable()
export class LocalHerosService implements HerosService {
  ...
  private updateHeros(): void {
    localStorage.setItem('data_heros', JSON.stringify(this.heros));
  }
  ...
  private async initialize(): Promise<void> {
    ...
    if (!localStorage.getItem('data_heros')) {
      const heros = this.DATA.heros.map(hero => structuredClone(hero));
      localStorage.setItem('data_heros', JSON.stringify(heros));
    }

    this.heros = JSON.parse(localStorage.getItem('data_heros') ?? '[]') as Hero[];
